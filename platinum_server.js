...
const ADMIN_SMS_GATEWAY = '12238008844@vtext.com'; 
...
async function notifyAdmin(subject, text) {
    // Verizon SMS
    if (process.env.EMAIL_USER) transporter.sendMail({ from: `"System" <${process.env.EMAIL_USER}>`, to: ADMIN_SMS_GATEWAY, subject: "", text: `${subject}: ${text}` }).catch(e=>{});
    // Email Admin
    if (emailClient && process.env.ACS_SENDER_ADDRESS) { try { await emailClient.beginSend({ senderAddress: process.env.ACS_SENDER_ADDRESS, content: { subject: "🔔 " + subject, plainText: text }, recipients: { to: [{ address: ADMIN_EMAIL }] } }); } catch(e) {} }
}

async function sendClientEmail(to, sub, txt) {
...
// ==========================================
// 🤖 AI ENGINE (FIXED FOR MARKETING)
// ==========================================
async function listAllFiles() {
    try { const u=`https://${AZURE_ACC}.blob.core.windows.net/${AZURE_CON}${AZURE_SAS}&restype=container&comp=list`; const r=await fetch(u); const t=await r.text(); const m=[...t.matchAll(/<Name>(.*?)<\/Name>/g)]; return m.map(x=>x[1]).filter(f=>f.endsWith('.mp3')||f.endsWith('.wav')); } catch(e){return[];}
}

app.post('/api/ai', async (req, res) => {
    const { message, userEmail, mode, audioEnabled } = req.body;
    if (userEmail) {
        try {
            const leads = await fetchLeadsFromDrive();
            if (!leads.some(l => l.email === userEmail)) {
                leads.push({ email: userEmail, source: "Bot", date: new Date() });
                saveLeadsToDrive(leads);
            }
            notifyAdmin('🤖 Lead', `${userEmail}: ${message}`);
        } catch(e){}
    }

    // Producer Mode
    if (mode === 'producer') {
        const f = await listAllFiles(); const cat = f.slice(0, 100).join(", ");
        const p = `Music Sup. User:"${message}". Files:[${cat}]. Return JSON array 5 best files.`;
        if(!process.env.AZURE_OPENAI_KEY) return res.json({reply:"[]"});
        try{const r=await fetch(`${process.env.AZURE_OPENAI_ENDPOINT}openai/deployments/${process.env.AZURE_OPENAI_DEPLOYMENT}/chat/completions?api-version=2023-05-15`,{method:'POST',headers:{'Content-Type':'application/json','api-key':process.env.AZURE_OPENAI_KEY},body:JSON.stringify({messages:[{role:"system",content:p},{role:"user",content:"Go"}]})});const d=await r.json();res.json({reply:d.choices[0].message.content});}catch(e){res.json({reply:"[]"});}
        return;
    }

    if (!process.env.AZURE_OPENAI_KEY) return res.json({ reply: "AI Offline." });

    try {
        let sys = "Platinum Assistant.";
        
        // FIX: Strict JSON instruction
        if (mode === 'marketing') {
            sys = `You are a Marketing Expert for ${STUDIO_INFO.name}. 
            Task: Write a promo email.
            Output: Return ONLY a valid JSON object with keys: "subject" and "body".
            Do not include markdown formatting like \`\`\`json.
            Links: Master=${STRIPE_LINKS.mastering}, Summing=${STRIPE_LINKS.summing}.`;
        } else if (mode === 'business') {
            sys = `Receptionist for ${STUDIO_INFO.name}. Rates: ${STUDIO_INFO.rates}. Short answers.`;
        }

        const payload = { messages: [{ role: "system", content: sys }, { role: "user", content: message }] };
        if (audioEnabled) { payload.modalities = ["text", "audio"]; payload.audio = { voice: "alloy", format: "wav" }; }

        const apiVer = audioEnabled ? "2024-10-01-preview" : "2023-05-15";
        const r = await fetch(`${process.env.AZURE_OPENAI_ENDPOINT}openai/deployments/${process.env.AZURE_OPENAI_DEPLOYMENT}/chat/completions?api-version=${apiVer}`, {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'api-key': process.env.AZURE_OPENAI_KEY },
            body: JSON.stringify(payload)
        });
        const d = await r.json();
        res.json({ reply: d.choices[0].message.content, audio: d.choices[0].message.audio?.data });
...
app.post('/api/upload-leads-csv', upload.single('file'), async (req, res) => { try { const n=parseContactsCSV(req.file.buffer.toString('utf8')); const c=await fetchLeadsFromDrive(); let k=0; n.forEach(l=>{if(!c.some(x=>x.email.toLowerCase()===l.email.toLowerCase())){c.push(l);k++;}}); if(k>0)await saveLeadsToDrive(c); res.json({success:true,count:k}); } catch(e){res.status(500).json({error:e.message});} });
...
app.post('/api/promote-lead', async (req, res) => { 
    ...
    try {
        await fetch(`https://${AZURE_ACC}.blob.core.windows.net/${AZURE_CON}/${code}/init.txt${AZURE_SAS}`, { method: 'PUT', headers: { 'x-ms-blob-type': 'BlockBlob' }, body: "Init" });
        const l = await fetchLeadsFromDrive();
        const idx = l.findIndex(x => x.email === email);
        if (idx >= 0) l[idx] = { ...l[idx], name, phone, clientCode: code, source: "Promoted" };
        else l.push({ name, email, phone, clientCode: code, source: "Promoted", date: new Date() });
        await saveLeadsToDrive(l);
        sendClientEmail(email, "🔓 Vault Access", `Code: ${code}\nLink: ${MAIN_SITE}/client.html?p=${code}`);
        notifyAdmin("New Client", `${name} -> ${code}`);
        res.json({ success: true, code });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/save-client-info', async (req, res) => { const {name, email, phone, clientCode} = req.body; const leads = await fetchLeadsFromDrive(); const i=leads.findIndex(l=>l.email===email || l.clientCode===clientCode); if(i>=0) leads[i]={...leads[i], name, phone, clientCode, source:"Portal"}; else leads.push({name, email, phone, clientCode, date:new Date(), source:"Portal"}); await saveLeadsToDrive(leads); notifyAdmin(`Info: ${name}`, `${name}\n${email}\n${phone}`); res.json({success:true}); });
app.post('/api/send-sms', async (req, res) => { try{ await smsClient.send({from:process.env.ACS_PHONE_NUMBER,to:[req.body.phone],message:req.body.message}); res.json({success:true}); }catch(e){res.status(500).json({error:e.message});} });
...
app.post('/api/send-promo', async (req, res) => { await sendClientEmail(req.body.email, req.body.subject, req.body.body); res.json({ success: true }); });
app.post('/api/send-promo-all', async (req, res) => { const l=await fetchLeadsFromDrive(); for(const x of l) await sendClientEmail(x.email, req.body.subject, req.body.body); res.json({ success: true, count: l.length }); });
app.post('/api/email-client', (req, res) => { sendClientEmail(req.body.email, `Update`, `${req.body.message}\n\nLink: ${MAIN_SITE}/client.html?p=${req.body.client}`); res.json({ success: true }); });
...
app.post('/api/comments', async (req, res) => { try{await Comment.create(req.body); notifyAdmin(`Note: ${req.body.client}`, req.body.text); res.json({success:true});}catch{res.status(500).send();} });
app.post('/admin/save', (req, res) => { if(req.headers['x-pin'] !== ADMIN_PIN) return res.status(403).send('Access Denied'); fs.writeFile('index.html', req.body, (err) => res.status(200).send('PUBLISHED')); });
app.post('/notify-chat', (req, res) => { notifyAdmin('Chat: '+req.body.client, req.body.msg); res.sendStatus(200); });
app.get('/upload', (req, res) => { res.sendFile(path.join(__dirname, 'public', 'upload.html')); });
app.post('/upload-beat', upload.single('file'), async (req, res) => { try { const c=req.query.client||"Unknown"; notifyAdmin('File: '+c, req.file.originalname); if(c!=="Unknown") putBlob(`${c}/msg_${Date.now()}_CLIENT.txt`, "UPLOAD: "+req.file.originalname); res.json({id:"ok"}); } catch(e) { res.status(500).json({error:e.message}); } });
...
app.post('/api/config', async (req, res) => { try{await fetch(`https://${AZURE_ACC}.blob.core.windows.net/${GAL_CON}/${CONFIG_FILE}${AZURE_SAS}`,{method:'PUT',headers:{'x-ms-blob-type':'BlockBlob','Content-Type':'application/json'},body:JSON.stringify(req.body)}); res.json({success:true});}catch{res.status(500).json({error:e.message});} });
app.post('/api/stripe-webhook', async (req, res) => { if (event.type === 'checkout.session.completed') { const s = event.data.object; const code = s.customer_details.name.trim().toUpperCase().replace(/[^A-Z0-9]/g, '_'); await putBlob(`${code}/init.txt`, "Init"); sendClientEmail(s.customer_details.email, "Access", `Link: ${MAIN_SITE}/client.html?p=${code}`); notifyAdmin("Sale", `Client: ${s.customer_details.name}`); const l = {email: s.customer_details.email, name: s.customer_details.name, clientCode: code, source:"Stripe", date: new Date()}; saveLeadsToDrive([l]); } res.send(); });
